...
## ✅ Investigação e redução de consumo de comandos Redis (aviso de limite do Upstash) (concluído em 2026-08-29)

- **Gatilho**: aviso do Upstash de que o plano free (500 mil comandos/mês) foi atingido. Confirmado ao vivo durante a investigação: o Redis de produção já estava em 500000/500000 no momento em que uma leitura extra (`XRANGE`) foi tentada.
- **Achado principal**: com apenas 2 linhas de produção `ATIVA` hoje (intervalos de 12-24h — dado real lido do Postgres de produção), o volume de trabalho de verdade (geração de artigo) é pequeno demais pra explicar o consumo. A causa é quase toda "custo de fundo" do worker BullMQ rodando 24/7: o maior de longe (~260 mil comandos/mês, ~52% do total) é o long-poll `BZPOPMIN` que o BullMQ usa internamente pra esperar por jobs `delayed`, limitado a blocos de 10s por uma constante fixa da própria lib (não configurável) — reemitido o dia inteiro mesmo com o próximo job agendado pra horas depois. Não é ajustável sem trocar a arquitetura de agendamento; documentado como decisão pendente em `DECISIONS.md`.
- **Confirmado que a hipótese inicial (polling do dashboard) não se aplica**: o Dashboard é Server Component puro, sem `setInterval`/`useSWR`/polling client-side nenhum.
- **Bug real encontrado em produção durante a investigação**: o contador de semáforo por provedor de IA (`provider-slot:openrouter`, usado por `withProviderSlot`/RF-30) estava com valor **negativo (-2) e sem TTL** — vazamento causado pelo padrão antigo de `INCR`/`DECR` não-atômico (uma chave que perde o TTL por um crash no meio do caminho nunca mais se autocorrige sozinha). Corrigido consolidando a aquisição num script Lua atômico e fazendo a liberação sempre renovar o TTL, garantindo autocorreção mesmo se acontecer de novo.
- **Otimizações de baixo risco implementadas** (todas configuráveis via env, sem exigir novo deploy pra recalibrar):
  - `stalledInterval`/`lockDuration` do `Worker` BullMQ, antes no default da lib (30s, rodando 24/7 mesmo ocioso), agora 120s — maior redução isolada implementada (~65 mil comandos/mês).
  - Heartbeat do worker no Redis (badge "online" do Dashboard): 30s → 60s.
  - Log periódico de saúde do worker (`getJobCounts`, era instrumentação pontual de um bug já corrigido em 2026-08-27): 5min → 30min.
  - Semáforo de concorrência por provedor de IA: `INCR`+`EXPIRE`+`DECR` separados → 1 script Lua atômico (também corrige o bug do parágrafo acima).
  - `scheduleLineRun`: removida uma leitura extra de confirmação (`job.getState()`) que só existia pra depurar um bug de 2026-08-27 já corrigido e coberto por teste de integração.
  - Web: removida uma chamada duplicada de `checkRateLimit` no login (contava a mesma tentativa 2x quando o e-mail não existia) e evitada uma leitura de TTL desnecessária quando a tentativa já é permitida.
- **Contador de comandos Redis por categoria implementado** (`packages/shared/src/redis-metrics.ts`): conta comandos por nome (incluindo os disparados internamente pelo BullMQ, e os de conexões duplicadas internamente) num objeto em memória, sem custo de Redis adicional. Aparece no log estruturado `heartbeat` do worker (`comandosRedis`, a cada 30min) — visibilidade real de consumo daqui pra frente, sem depender só do aviso por e-mail do Upstash.
